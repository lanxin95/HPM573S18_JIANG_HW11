...
def draw_survival_curves_and_histograms(simOutputs_none, simOutputs_anti):
    """ draws the survival curves and the histograms of time until deaths
    :param simOutputs_none: output of a cohort simulated under mono therapy
    :param simOutputs_anti: output of a cohort simulated under combination therapy
    """
...
def print_comparative_outcomes(simOutputs_none, simOutputs_anti):
    ...
    # increase in times of stroke
    increase_stroke_time = Stat.DifferenceStatIndp(
        name='Increase in survival time',
        x=simOutputs_none.get_if_developed_stroke(),
        y_ref=simOutputs_anti.get_if_developed_stroke())

    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=increase_stroke_time.get_mean(),
        interval=increase_stroke_time.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average increase in times of stroke "
          "and {:.{prec}%} confidence interval:".format(1 - Settings.ALPHA, prec=0),
          estimate_CI)
    ...
    increase_discounted_cost = Stat.DifferenceStatIndp(
        ...
        y_ref=simOutputs_anti.get_costs())
    ...
    increase_discounted_utility = Stat.DifferenceStatIndp(
        ...
        y_ref=simOutputs_anti.get_utilities())
...
def report_CEA_CBA(simOutputs_none, simOutputs_anti):
    """performs cost-effectiveness and cost-benefit analyses
    :param simOutputs_none:
    :param simOutputs_anti:
    :return:
    """
    ...
    combo_therapy_strategy = Econ.Strategy(
        ...
        cost_obs=simOutputs_anti.get_costs(),
        effect_obs=simOutputs_anti.get_utilities()
    )
